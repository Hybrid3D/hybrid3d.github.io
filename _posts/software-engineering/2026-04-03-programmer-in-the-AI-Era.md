...
고용주가 직접 AI를 써서 문제가 잘못되면 누굴 탓할 것인가? AI가 만든 코드가 서버를 터트리고, 아이템이나 돈 같은 재화를 복제할 때 고용주는 누구를 탓할 것인가? OpenAI? Claude? Google 같은 AI 제공자를 탓할 것인가? 그들이 책임을 져줄 것인가 하면 그렇지 않다. 책임은 AI 사용자의 몫이다. 며칠전 있었던 공영 방송사 자동 번역 사고[^2]를 포함해서 여러 사건 사고들을 보면 AI는 책임을 지지 않고 질 수도 없다는 것을 알 수 있다.
...
John Carmack은 누군가와의 트위터 DM 대화에서 다음과 같은 말을 한적이 있다[^3].

...
이런 경우 브랜칭이 별거 아닌 것처럼 오해할지도 모른다. 이렇게 보면 대입 두번 정도 하는 것이 별거 아닌 것처럼 보이지만 모든 분기를 다 계산한다는 것은 근본적으로는 전혀 달라지지 않는다. 

이 때 어셈블리 수준에서는 IF + mov 명령어가 movc 하나로 바뀌는데 어셈블리 수준에서 IF + mov 명령어가 느린 것은 아니기 때문에 이 명령어 몇개 줄어든다고 차이가 성능 차이를 가져올 가능성은 사실상 없고 근본적으로 movc 명령어로 바뀐다고 다이나믹 브랜칭이 사라지는 것 또한 아니다(대입의 개수는 달라지지 않는다). 결과적으로 이렇게 하는 것은 성능상 아무런 이점이 없을 뿐더러 심지어 더 느려지는 요소가 된다. 느려지는 이유는 이런 코드가 GPU의 수행 최적화를 방해하는 것인데(아래 섹션 '생각보다는 똑똑한 GPU' 참고) 실제로 느려지는 것을 확인한 이 테스트 결과는 다음 글에서 자세히 다루겠다.
...
다시 정리하자면 GPU에서는 분기 문에서 IF/ELSE 코드를 둘다 수행하는 상황이 생기면서 버리는 처리를 해버린다. 이 때 관련 된 처리가 비싸다면 불필요한 성능 하락이 있을 수 있다.
따라서 여러 비싼 처리를 분기 처리할 때는 매우 조심해야한다. 이에 대한 해결 방법은 여러가지가 있지만 항상 그렇게 단순한 것은 아니기 때문에 위험성을 항상 염두하는 것이 중요하다.

...
이 때 어셈블리 수준에서는 if + mov 명령어가 movc 하나로 바뀌는데 어셈블리 수준에서 **if + mov 명령어가 느린 것은 아니기 때문에** 이 명령어 몇개 줄어든다고 차이가 성능 이점을 가져올 가능성은 사실상 없다. 오히려 분기 분산을 명확하게 만들어버리기 때문에 성능에 큰 하락을 가져온다.
